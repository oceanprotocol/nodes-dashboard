 ...
 * served by the node (getServiceTemplates); see useServiceTemplates. Templates (the same apps with models
 * pre-loaded — `kind: 'bundle'` on the wire) are filtered out here; they have their own catalogue at
 * /inference/templates.
 */
const ServicesPage: React.FC = () => {
  ...
  useEffect(() => {
    ...
    router.replace({ pathname: '/inference/services', query }, undefined, { shallow: true });
  ...
  const summary = useMemo(() => {
    ...
    const lead = parts.length > 0 ? `${parts.join(' · ')} — ` : 'All services — ';
    return `${lead}${visible.length} of ${templates.length} services`;
  ...
  const emptyHint = useMemo(() => {
    ...
    if (filters.category !== 'all' && filters.hardware !== 'all') {
      return `No ${CATEGORY_META[filters.category].label} service that runs on ${HARDWARE_LABEL[filters.hardware]} is available right now.`;
    }
    if (filters.category !== 'all') {
      return `No ${CATEGORY_META[filters.category].label} service is available right now.`;
    }
    if (filters.hardware !== 'all') {
      return `No service that runs on ${HARDWARE_LABEL[filters.hardware]} is available right now.`;
    }
  ...
  const renderToolbar = () => (
    <>
      <div className={styles.headerRow}>
        <div className={styles.headerText}>
          ...
          <div className="textSecondary">
            Ready-made containerized apps — pick one, review what&apos;s inside, choose an environment, pay and launch. Models are yours to add from the app once it&apos;s running.
          </div>
        </div>
        <div className={styles.toolbarControls}>
          <Input
            ...
            value={filters.query}
          />
          <div aria-label="Filter services by hardware" className={styles.segmented} role="group">
      ...
      <div aria-label="Filter services by category" className={styles.categoryRow} role="group">
  ...
  return (
    <Container className="pageRoot">
      <SectionTitle
        ...
        contentBetween={<InferenceStepper currentStep="template" flowType={InferenceFlowType.Template} kindLabel="Service" />}
      />
      <div className="pageContentWrapper">
        <Card direction="column" padding="md" radius="lg" shadow="black" spacing="md" variant="glass-shaded">
          {loading ? (
            ...
            <div className={cx(styles.stateBox, 'textSecondary')}>No services available.</div>
          ) : (
            <>
              {renderToolbar()}
              {visible.length > 0 ? (
                <div className={styles.grid}>
                  {visible.map((item) => (
                    <ServiceCard item={item} key={item.tpl.id} onOpen={openDetails} />
                  ))}
                </div>
              ) : (
                <div className={styles.emptyState}>
                  <FilterAltOffIcon className={styles.emptyIcon} />
                  <div className={styles.emptyTitle}>No services match these filters</div>
